...
def click_train(
    ...
    # 生成config#无需生成config
    # cmd = python_cmd + " train_nsf_sim_cache_sid_load_pretrain.py -e mi-test -sr 48k -f0 2 -bs 4 -g 0 -te 10 -se 5 -pg pretrained/f0G48k.pth -pd pretrained/f0D48k.pth -l 1 -c 0"
...
with gr.Blocks(theme=gr.themes.Base()) as app:
    with gr.Tabs():
        with gr.TabItem("Inference"):
            gr.HTML("<h1> Easy GUI v2 (rejekts) - адаптирован под Mangio-RVC-Fork и переведен 💻 </h1>")
            ...
            with gr.Row():
                ...
                vc_transform0 = gr.Number(label="Необязательно: здесь вы можете изменить высоту тона (-12 сделает голос более мужским, 12 более женским).", value=0)
            ...
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        dropbox = gr.File(label="Переместите свое аудио сюда & Нажмите на кнопку для перезагрузки.")
                    with gr.Row():
                        record_button=gr.Audio(source="microphone", label="ИЛИ Запишите аудио.", type="filepath")
                    with gr.Row():
                        input_audio0 = gr.Dropdown(
                            label="2.Выберите свое аудио.",
                    ...
                    with gr.Row():
                        with gr.Accordion('Text To Speech', open=False):
                            with gr.Column():
                                ...
                                api_box = gr.Textbox(label="Введите ключ API для ElevenLabs или оставьте поле пустым, чтобы использовать GoogleTTS.", value='')
                                elevenid=gr.Dropdown(label="Голос:", choices=eleven_voices)
                            with gr.Column():
                                tfs = gr.Textbox(label="Введите текст", interactive=True, value="This is a test.")
                ...
                with gr.Column():
                    with gr.Accordion("Index Settings", open=False):
                        file_index1 = gr.Dropdown(
                            label="3. Путь к добавленному файлу .index (если он не выбран автоматически).",
                            choices=get_indexes(),
                            value=get_index(),
                            interactive=True,
                            )
                    ...
                    vc_output2 = gr.Audio(label="Вывод аудио (нажмите на три точки справа, чтобы скачать)",type='filepath')
...
=======
                            label="Необязательно: измените алгоритм извлечения высоты тона.",
                            choices=["pm", "dio", "mangio-crepe-tiny", "crepe-tiny", "crepe", "mangio-crepe", "harvest"], # Fork Feature. Add Crepe-Tiny
>>>>>>> 18dd669afaccd71cfb798dd61778a95fdbf3ad1e
                            value="mangio-crepe",
                            interactive=True,
                        )
                        crepe_hop_length = gr.Slider(
                            ...
                            maximum=512,
                            step=1,
                            label="Mangio-Crepe Hop Length. Более высокие значения уменьшат вероятность экстремальных изменений высоты тона, но более низкие значения повысят точность.",
                            value=120,
                            interactive=True
                            )
        ...
        with gr.TabItem("Download Model"):
            with gr.Row():
                url=gr.Textbox(label="Вставьте ссылку на модель:")
            with gr.Row():
        ...
        with gr.TabItem("Train", visible=False):
            with gr.Row():
                with gr.Column():
                    exp_dir1 = gr.Textbox(label="Название исполнителя (модели):", value="My-Voice")
                    sr2 = gr.Radio(
                        label=i18n("目标采样率"),
                        choices=["40k", "48k"],
                        value="40k",
                    ...
                    np7 = gr.Slider(
                        ...
                        step=1,
                        label="Количество процессоров для обработки данных (НЕ ТРОГАТЬ!)",
                        value=config.n_cpu,
                        interactive=True,
                        visible=True
                    )
                    trainset_dir4 = gr.Textbox(label="Путь к вашему датасету (аудио, не в архиве):", value="/kaggle/working/dataset")
                    easy_uploader = gr.Files(label='ИЛИ Перетащите сюда свои аудиозаписи. Они будут загружены по указанному выше пути в ваш датасет.',file_types=['audio'])
                    but1 = gr.Button("1.Process The Dataset", variant="primary")
                    info1 = gr.Textbox(label="Статус (подождите, пока не появится 'end preprocess'):", value="")
                ...
                with gr.Column():
                    ...
                    info2 = gr.Textbox(label="Статус (проверьте вывод ячейки Kaggle):", value="", max_lines=8)
                    but2.click(
                            extract_f0_feature,
                ...
                with gr.Row():      
                    with gr.Column():
                        total_epoch11 = gr.Slider(
                            ...
                            maximum=10000,
                            step=10,
                            label="Общее количество эпох обучения (ЕСЛИ вы выберете слишком большое значение, ваша модель будет перетренированной):",
                            value=250,
                            interactive=True,
                        )
                        but3 = gr.Button("3.Train Model", variant="primary")
                        but4 = gr.Button("4.Train Index", variant="primary")
                        info3 = gr.Textbox(label="Статус (проверьте вывод ячейки Kaggle):", value="", max_lines=10)
                        with gr.Accordion("Training Preferences (You can leave these as they are)", open=False):
                            ...
                            with gr.Column():
                                ...
                                batch_size12 = gr.Slider(
                                    ...
                                    maximum=40,
                                    step=1,
                                    label="Batch Size (НЕ ТРОГАЙТЕ ЭТО, если не знаете, что делаете!):",
                        ...
                        zipped_model = gr.Files(label='Ваш файл модели и индекс можно скачать здесь:')
